...
namespace PTIC.Marketing.Report
{
    public partial class frmMonthlyCustomerComplainSummary : Form
    {
        ...
        {
            string templateSql = @"SELECT     (SELECT COUNT(*) FROM ComplaintCase WHERE ID<=CC.ID) AS [No],CC.CaseDespt, ISNULL(CASEINMONTH.RefNo, '') AS COMPLAINNO, ISNULL(CASEINMONTH.ProductName, '') AS PRODUCT, ISNULL(CASEINMONTH.Qty, 0) AS QTY
            ...
            reportViewer1.LocalReport.DataSources.Add(new ReportDataSource("MonthlyCustomerComplain", dt));
