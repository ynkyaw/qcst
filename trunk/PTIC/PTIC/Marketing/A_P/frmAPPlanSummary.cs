...
namespace PTIC.VC.Marketing.A_P
{
    public partial class frmAPPlanSummary : Form
    {
        ...
        private void SearchNBind(DateTime startDate, DateTime endDate)
        {
            ...
            DataTable dtFormattedSummary = CreateSummaryTableStructure(startDate.Month);
            foreach (DataRow row in dtSummary.Rows)
            {
                DateTime planDate = (DateTime)DataTypeParser.Parse(row["A_P_PlanDate"], typeof(DateTime), DateTime.Now);
                int monthNo = 0;
                switch (planDate.Month)
                {
                    case (int)PTIC.Common.Enum.MonthOfYear.January:
                        {
                            monthNo = (int)PTIC.Common.Enum.MonthOfYear.January;
                            break;
                        }
                    case (int)PTIC.Common.Enum.MonthOfYear.February:
                        {
                            monthNo = (int)PTIC.Common.Enum.MonthOfYear.February;
                            break;
                        }
                    case (int)PTIC.Common.Enum.MonthOfYear.March:
                        {
                            monthNo = (int)PTIC.Common.Enum.MonthOfYear.March;
                            break;
                        }
                    case (int)PTIC.Common.Enum.MonthOfYear.April:
                        {
                            monthNo = (int)PTIC.Common.Enum.MonthOfYear.April;
                            break;
                        }
                    case (int)PTIC.Common.Enum.MonthOfYear.May:
                        {
                            monthNo = (int)PTIC.Common.Enum.MonthOfYear.May;
                            break;
                        }
                    case (int)PTIC.Common.Enum.MonthOfYear.June:
                        {
                            monthNo = (int)PTIC.Common.Enum.MonthOfYear.June;
                            break;
                        }
                    case (int)PTIC.Common.Enum.MonthOfYear.July:
                        {
                            monthNo = (int)PTIC.Common.Enum.MonthOfYear.July;
                            break;
                        }
                    case (int)PTIC.Common.Enum.MonthOfYear.August:
                        {
                            monthNo = (int)PTIC.Common.Enum.MonthOfYear.August;
                            break;
                        }
                    case (int)PTIC.Common.Enum.MonthOfYear.September:
                        {
                            monthNo = (int)PTIC.Common.Enum.MonthOfYear.September;
                            break;
                        }
                    case (int)PTIC.Common.Enum.MonthOfYear.October:
                        {
                            monthNo = (int)PTIC.Common.Enum.MonthOfYear.October;
                            break;
                        }
                    case (int)PTIC.Common.Enum.MonthOfYear.November:
                        {
                            monthNo = (int)PTIC.Common.Enum.MonthOfYear.November;
                            break;
                        }
                    case (int)PTIC.Common.Enum.MonthOfYear.December:
                        {
                            monthNo = (int)PTIC.Common.Enum.MonthOfYear.December;
                            break;
                        }
                } // END of switch (planDate.Month)
                DataRow[] fomattedRows = dtFormattedSummary.Select(string.Format("A_P_MaterialID = {0}",
                                            (int)DataTypeParser.Parse(row["A_P_MaterialID"], typeof(int), -1)));
                if (fomattedRows.Length > 0) // if A P has already existed
                {
                    DataRow fRow = fomattedRows[0];
                    fRow["PlanAmt" + monthNo] = row["PlanAmt"];
                    fRow["PrevMonthBalAmt" + monthNo] = row["PrevMonthBalAmt"];
                    fRow["AvailableAmt" + monthNo] = row["AvailableAmt"];
                    fRow["UsedAmt" + monthNo] = row["UsedAmt"];
                    fRow["ThisMonthBalAmt" + monthNo] = row["ThisMonthBalAmt"];

                    //DataColumn colPlanAmt = new DataColumn("PlanAmt" + i, typeof(decimal));
                    //DataColumn colPrevMonthBalAmt = new DataColumn("PrevMonthBalAmt" + i, typeof(decimal));
                    //DataColumn colAvailableAmt = new DataColumn("AvailableAmt" + i, typeof(decimal));
                    //DataColumn colUsedAmt = new DataColumn("UsedAmt" + i, typeof(decimal));
                    //DataColumn colThisMonthBalAmt = new DataColumn("ThisMonthBalAmt" + i, typeof(decimal));
                }
                else // if A P has not been in formatted summary table
                {
                    if (monthNo != 0)
                    {
                        DataRow formattedRow = dtFormattedSummary.NewRow();
                        formattedRow["A_P_MaterialID"] = row["A_P_MaterialID"];

                        formattedRow["PlanAmt" + monthNo] = row["PlanAmt"];
                        formattedRow["PrevMonthBalAmt" + monthNo] = row["PrevMonthBalAmt"];
                        formattedRow["AvailableAmt" + monthNo] = row["AvailableAmt"];
                        formattedRow["UsedAmt" + monthNo] = row["UsedAmt"];
                        formattedRow["ThisMonthBalAmt" + monthNo] = row["ThisMonthBalAmt"];

                        dtFormattedSummary.Rows.Add(formattedRow);
                    }
                }
                //startMonth++;
            }// END of foreach(DataRow row in dtSummary.Rows)
            dgvAPPlanSummary.DataSource = dtFormattedSummary;
        }
